...
\begin{rmk}
  ...
  side ofa hyperplane in Euclidean space and forming pairwise obstuse
...
\begin{proof}
  ...
  \ref{}, \(\W\) acts transitively on the chambers. Thus, each orbit
...
\begin{prop}
  ...
  only one dominant weight. If \(\lambda\) is dominant, then \(\sigma
  \lambda \prec \lambda\) for all \(\sigma \in \W\), and if
  \(\lambda\) is strongly dominant, then \(\sigma \lambda = \lambda\)
  only when \(\sigma = 1\).
\end{prop}
\begin{proof}
  This result follows from
  \ref{weyl-chamber-closure-contains-one-point-in-orbit} and \ref{}
\end{proof}
\begin{bibdiv}
  \begin{biblist}
    \bib{humph}{book}{
