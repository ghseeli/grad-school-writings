...
\begin{defn}
  ...
  fundamental dominant weights are such that \(\langle \omega_i, \alpha_j \rangle = \delta_{i,j}\).
\end{defn}
\begin{defn}
...
\end{prop}
\begin{proof}
  Write \(\alpha_i = \sum_j m_{i,j} \omega_j\) for \(m_{i,j} \in
  \Z\). Then, we see
  \begin{equation}
    \label{eq:weight-decomp}
    \langle \alpha_i, \alpha_k \rangle = \sum_{j} m_{i,j} \langle
    \omega_j, \alpha_k \rangle = m_{i,k} = \langle \alpha_i, \omega_k \rangle
  \end{equation}
