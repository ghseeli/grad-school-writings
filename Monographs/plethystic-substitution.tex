...
\begin{rmk}
  ...
  the constants \de{binomial variables}. So, in the proposition above,
...
\begin{proof}
  ...
  (1-t^{n}) = 1\), so we are done. \todo{Of course, I do not
    understand why this should be true; certainly, it would not work
    for my calculus students. Perhaps since we are using an
    expansion where t < 1 anyways, this follows. } 
\end{proof}
\begin{prop}
...
% In the study of the representation theory of the symmetric group, one
% useful fact is that there is an isometry \(\ch \from R \to \sym
% \otimes_\Z \C\) where \(R = \bigoplus_n R^{(n)}\) and \(R^{(n)}\) the
% free \(\Z\)-module generated by the irreducible characters of
% \(\Sym_n\). Amazingly, this isometry can be used to understand
% plethysm in representation theoretic terms.
% \begin{defn}
%   Let \(u,v \in R\). Then, define \(u[v] = u \circ v := \ch^{-1}(\ch u
%   \circ \ch v) = \ch^{-1}((\ch u)[\ch v])\). 
% \end{defn}
% \begin{prop}
%   Given \(u,v \in R\),
%   \begin{enumerate}
%   \item \(u \circ v\) is a character of \(\Sym_{mn}\).
%   \item 
%   \end{enumerate}
% \end{prop}
\begin{bibdiv}
  \begin{biblist}
    ...
    \bib{stanley}{book}{
      author={Stanley, R.P.}
      title={Endumerative Combinatorics, Vol 2}
      year={1999}
    }
  \end{biblist}
\end{bibdiv}
