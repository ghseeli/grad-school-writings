...
\begin{rmk}
  ...
  reflection \(s_i\) on a positive set of roots \(R_+\) takes the set
  \(R_+ \setminus \{\alpha_i\}\) to \(R_+ \setminus
...
\begin{proof}[Proof of Gabriel's Theorem (continued)]
  ...
  and thus, by \ref{relate-refl-to-refl-func}, \[
    \grdim V = s_{i_1} \cdots s_{i_k}(\alpha_{i_{k+1}}) \in R_+
  \]
...
\begin{defn}
  ...
  \cdots s_{i_1}\) reduced, where \(N = \ell(w_\circ) = |R_+|\). 
\end{defn}
\begin{rmk}
...
\end{defn}
\begin{prop}
  \(R_+ = \{\gamma_1, \ldots, \gamma_N\}\). This is a special case of
  \(R_+(W)\). \todo{Why? Any intuition at least?}
\end{prop}
\begin{prop}
...
\subsection*{(9/21/2017) Lecture 9}
...
\begin{defn}
  ...
  in \(\Rep(\Q)\) such that \[
    [M_1] \cdot [M_2] = \sum_{[L]} F_{M_1,M_2}^L [L]
  \]
\end{defn}
\begin{prop}
...
\begin{proof}
  To see associativity, note that \[
    ([M_1] \cdot [M_2]) \cdot [M_3] = F_{M_1, M_2, M_3}^L = [M_1]
    \cdot ([M_2] \cdot [M_3])
  \]
\end{proof}
\begin{rmk}
  There is also a twisted product, given by \[
    U \circ V = q^{\frac{\langle \vec{u}, \vec{v} \rangle}{2} U \cdot V}
  \]
...
\begin{align*}
  \{\text{Indecomposables}\} & \onetoonecorrespondance R_+ \text{
                               (Positive roots)} \\
  I_\alpha & \mapsfrom \alpha \\
  \text{Isomorphism classes in } \Rep(\Q) & \correspondsto \N^{R_+} :=
                                            \{\vec{n} \from R_+ \to
                                            \N\} \\
  M_n = \bigoplus_{\alpha \in R_+} n_\alpha I_\alpha
  & \mapsfrom \vec{n} = (n_\alpha)_{\alpha \in R_+}
\end{align*}
\begin{prop}
  Let \(\Q\) be Dynkin. Then, for all \(\vec{n}, \vec{m}, \vec{k} \in
  \N^{R_+}\), there is a polynomial \(\phi_{\vec{n},\vec{k}}^{\vec{m}}
  \in \Z[t]\) such that \[
...
H(\Q)_{\Z[t]}\) with multiplication \[
  [M_{\vec{n}}] \cdot [M_{\vec{k}}] = \sum_{\vec{m}}
...
\]
\begin{thm}
  Let \(\tilde{A} = \Z[t][[n]_t^{-1}, \forall n \geq 1]\). Then, the
  (Ringel-)Hall algebra \(H(\Q)_{\tilde{A}}\) is generated by \(\theta_i =
  [S(i)]\) for \(i \in I\). 
\end{thm}
