...
\title[]{Catalanimals, shuffle theorems, and Macdonald polynomials} % The short title appears at the bottom of every slide, the full title is only on the title page
...
\medskip
SMRI: Modern Perspectives in Representation Theory\\ % Your institution for the title page
\medskip
%Based on arXiv:2112.07063 and arXiv:2307.06517
}
\date{22 May 2025} % Date, can be changed to a custom date
...
\begin{frame}{Overview}
  \begin{center}
    \begin{tikzpicture}
      \onslide<1->{
      \node (alg) at (0,0) {Algebraic quantity};
      \node at (1.75,0) {\(=\)};
      \node (comb) at (5,0) {Combinatorial generating function};
      }
      \onslide<2->{
      \node (rep) at (0,-2) {Representation theory};
      \draw[->,dashed] (rep) -- (alg);
      }
      \onslide<3->{
      \node (closed) at (0,2) {Closed form expression};
      \node at (2.5, 2) {\(=\)};
      \node (ser) at (5,2) {Infinite symmetric series};
      \draw[->] (closed) -- (alg);
      \draw[->] (ser) -- (comb);
      }
      \onslide<4->{
      \node (cat) at (2.5, 3) {Catalanimals};
      \draw[->] (cat) -- (closed);
      \draw[->] (cat) -- (ser);
      }
    \end{tikzpicture}
...
%   \end{align*}\pause
%   \(\implies\) Basis of symmetric functions often indexed by partitions.
% \end{frame}
\begin{frame}{Symmetric polynomials and functions}
  \begin{itemize}
  \item Symmetric polynomials \(\Q[z_1,\ldots,z_n]^{S_n}\)
  \item Generators
    \[
...
\begin{frame}{A Graded Example}
   ...
   Consider \(S_3 \curvearrowright M = \) all partial derivatives of \(\Delta\).
   \[
   \begin{tikzcd}
...
% \begin{frame}{Getting more information}
%   \pause
%   \ytableausetup{boxsize=0.5em,aligntableaux=top}
%   \[
%     \hspace{-0.75em}
%     \scalebox{0.95}{\(
%       \underbrace{\Span\{\Delta\}}_{V_{\ydiagram{1,1,1}}} {\oplus} \underbrace{\Span\{2x_1(x_2{-}x_3){-}x_2^2{+}x_3^2,
%         2x_2(x_3{-}x_1){-}x_3^2{+}x_1^2\}}_{\substack{V_{\ydiagram{1,2}}\\\deg
%         = 2}} {\oplus}
...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}[xscale = 0.33,yscale = 0.33]
      ...
      \node at (12,0) {\textcolor{orange}{$\lambda$}};
      \foreach \point in
      {(0.05,10),(0.05,9),(1.05,9),(1.05,8),(2.05,8),(3.05,7),(3.05,6),(4.05,6),(8.05,2),(9.05,1)} {
        \fill[lightblue, visible on=<3->] \point -- +(0,-0.95) --
...
\begin{frame}{Generalizing Shuffle Theorem}
  ...
  %     {
  %       \fill[lightblue, visible on=<2->] \point -- +(0,-0.95) --
  %       +(0.95,-0.95) -- +(0.95,0) -- cycle;
  %     }
...
\begin{frame}{Elliptic Hall Algebra}
    ...
    \onslide<3->{
      \node at (-1,-2) {\(\displaystyle \mathcal{E} \underset{\text{v.sp.}}{\isom}
        \text{\parbox{10ex}{central \\ subalgebra}}
        \oplus \bigoplus_{m,n \text{ coprime}} \Lambda^{(m,n)}\)};
    }
    \onslide<5->{
      \node at (5,-2) {Each \(\Lambda^{(m,n)} \isom\) \parbox{12ex}{symmetric polynomials}};
      }
      \onslide<4->{
      \node at (5,0) {
    ...
    \begin{itemize}
    \onslide<6->{
    \item For \(f^{(m,n)} \in \Lambda^{(m,n)}\), 
      \(\nabla f^{(m,n)} \nabla^{-1} = f^{(m+n,n)}\) \vspace{-1ex}
    }
    \onslide<7->{
    \item  Algebraic side of Shuffle Theorem =
          \(e_k^{(1,1)} \in \Lambda^{(1,1)}\) acting on
          \(1 \in \Lambda\).
        }
    \onslide<8->{
    \item \(e_k^{(1,1)} \cdot 1 = \nabla e_k^{(0,1)} \nabla^{-1}
    ...
    \onslide<9->{
    \item Can be difficult to work with in general. Can we
...
\begin{frame}{Weyl symmetrization}
 ...
 \item Define \(\pol_X \chi_\lambda(\zz) = s_\lambda\) if \(\lambda_l \geq 0\),
   otherwise \(0\).
 \end{itemize}
\end{frame}
...
\begin{frame}{Why?}
  ...
  \begin{prop}
    ...
    \[e_k^{(m,n)} \cdot 1 = H(R_+, R_+, R_+^0, \bb)\] for \pause
...
\begin{tikzpicture}[xscale = 0.33,yscale = 0.33]
      ...
      \node at (0,6) {$\bullet$};
      \node at (11,0) {$\bullet$};
      \draw[thick] (0,6)--(0,5)--(1,5)--(1,4)--(3,4)--(3,3)--(5,3)--(5,2)--(7,2)--(7,1)--(9,1)--(9,0)--(11,0);
      \node at (12,1.5) {$\delta$};
...
\begin{frame}{Results}
 ...
 a proof of the Rational Shuffle Theorem + a generalization. 
  \begin{block}{Theorem (Blasiak-Haiman-Morse-Pun-S., 2023)}
    ...
    \[
      \onslide<2->{H(R_+,R_+,R_+^0,\bb)} \onslide<3->{=}
  ...
  \begin{columns}
    \begin{column}{0.45\textwidth}
      ...
      \onslide<1->{
      \draw[step=1cm,gray!20,very thin] (0,0) grid (11,6);
      \draw[step=1cm,gray!20,thin] (0,5.45)--(11.4,0);
      \draw[fill = black] (0,5.45) circle (0.15) node[left]
      {\(\scriptstyle (0,s)\)};
      \draw[fill = black] (11.4,0) circle (0.15) node[below]
      {\(\scriptstyle (r,0)\)};
      \draw[thick]
      (0,5) -- (0,4) -- (1,4)--(1,3)--(3,3)--(3,2)--(6,2)--(6,1)--(8,1)--(8,0)--(11,0);
      \node at (12,0.5) {$\lambda$};
      }
      \onslide<5->{
      \foreach \point in
      {(1.05,4),(2.05,4),(3.05,3),(4.05,3),(6.05,2),(8.05,1)}
      {
        \fill[lightblue, visible on=<2->] \point -- +(0,-0.95) --
        +(0.95,-0.95) -- +(0.95,0) -- cycle;
      }}
...
\draw[draw = none, fill = black!100] (2,-1) rectangle (3,-2);
\end{tikzpicture}} \
$R_+ \setminus R_q = $ pairs of boxes in the same diagonal in the same
shape,
\item[] \raisebox{-1mm}{
 \begin{tikzpicture}[scale = .4]
\draw[draw = none, fill = red!68] (2,-1) rectangle (3,-2);
...
\end{frame}
\begin{frame}{LLT Catalanimals}
\begin{theorem}[Blasiak-Haiman-Morse-Pun-S., 2024]
...
\vspace{-1.4mm}
\begin{conjecture}[Blasiak-Haiman-Morse-Pun-S., 2025]
...
\begin{frame}
  ...
  \item Shuffle algebra \(S\) given by the image of Laurent
    polynomials \(\phi \in \Q(q,t)[x_1^{\pm 1},\ldots, x_l^{\pm 1}]\) via
    map \[
