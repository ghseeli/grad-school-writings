...
\title[]{Flagged LLT polynomials and nonsymmetric Macdonald polynomials} % The short title appears at the bottom of every slide, the full title is only on the title page
...
\medskip
UC Davis Algebra \& Discrete Mathematics Seminar \\ % Your institution for the title page
\medskip
joint work with Jonah Blasiak, Mark Haiman, Jennifer Morse, and Anna Pun
}
\date{20 April 2026} % Date, can be changed to a custom date
...
\begin{frame}{Some notable occurrences of LLT Polynomials}
  ...
  \item For \(G = \) incomparability graph for natural unit interval
    ...
    polynomial is a ``signed'' LLT polynomial (later), i.e.,
    \[
...
\begin{frame}{Applications}
  ...
  \item Tewari-Wilson-Zhang define chromatic nonsymmetric polynomials
    ...
    steps, \(\chi_{\bb,\pi}\). This is (essentially) a signed
    flagged LLT polynomial.
    % \[
...
\begin{frame}{Nonsymmetric HHL formula example}
  ...
  For \(\alpha = (2,0,1)\), we get 8 signed flagged LLT terms:
  \begin{equation*}
...
%  \node (d) at (1,1.5) {\small modified};
  ...
  \draw[->] (B) -- node[midway, right] {\footnotesize }  (D);
\end{tikzpicture}
\end{center}
...
\end{frame}
% \begin{frame}{A story of two HHL's}
%   \begin{itemize}
%   \item Haglund-Haiman-Loehr give a monomial formula for
%     \(H_\mu(x;q,t)\) and another for \(E_\alpha(x;q,t)\) in terms of fillings.
%   \item Formulas have similar features.  
%   \item The \(H_\mu\) formula can be grouped into LLT
%     polynomials indexed by tuples of ribbons, \(\Gcal_R\).
%   \item In fact, the corresponding \(\Ecal_\alpha(x;q,t)\) formula can
%     be grouped into signed flagged LLT polynomials, \(\Gcal^{\pm}_\nubold\).
%   \item Idea: The missing corner should have an HHL formula using
%     (unsigned) flagged LLTs!
%   \end{itemize}
% \end{frame}
...
\begin{frame}{Modern Macdonald polynomials}
  \begin{definition}
    ...
    \]
  \end{definition}
    For \(r=2\), \(\eta=(2,1)\), \(\lambda = \emptyset\), the LLT
    terms are
...
\begin{frame}{Modern Macdonald polynomials}
  ...
  \item \(\nsH_{\eta | \lambda}(x;q,t)\) form a basis of \(\mathcal{P}(r)\).\pause
  \item We conjecture \(\nsH_{\eta | \lambda}(x;q,t)\) is atom positive
    (would follow from flagged LLTs being atom positive).
  \end{itemize}
\end{frame}
\begin{frame}{Modern Macdonald summary}
\begin{equation*}
...
\begin{frame}{Stabilization}
  \begin{definition}
    A sequence \(g_1,g_2,\ldots\) with \(g_N \in
    \Q(q,t)[x_1,\ldots,x_N]\) \emph{converges \(t\)-adically} to
    \(f(x) \in \Pcal(r)\) if, for all \(e \geq 0\),
    \[
      g_N(x_1,\ldots,x_N) - f(x_1,\ldots,x_N,0,0,\ldots)
    \]
    has coefficients whose order of vanishing in \(t\) is at least
    \(e\), for sufficiently large \(N\).
  \end{definition}
  \begin{itemize}
  \item \(1, 1+t, 1+t+t^2,\ldots \to \frac{1}{1-t}\).
  \item \(x_1, tx_1+x_2, t^2x_1+x_2+x_3, t^3 x_1+x_2+x_3+x_4, \ldots
    \to x_2+x_3+x_4+\cdots\).
  \end{itemize}
\end{frame}
\begin{frame}{Stable nonsymmetric Macdonald polynomials}
  \begin{definition}
    For $(\eta|\lambda) \in \N^r \times \Par$, the \emph{integral form
      stable $r$-nonsymmetric Macdonald polynomial}
    $\nsJ_{\eta|\lambda}(\xx;q,t) \in \Pcal(r)$ is given by
    \vspace{-1.4mm}
    \begin{align*}
      \nsJ_{\eta| \lambda}(\xx;q,t)
      = \lim_{n \to \infty} \Ecal_{(\eta; 0^{n}; \lambda)}(x_1,\dots,x_{r+n}, 0^{\ell(\lambda)};q,t).
    \end{align*}
  \end{definition}
  These \(\nsJ_{\eta|\lambda}\)'s are integral forms of stable
  versions of \(E_\alpha\) introduced by Bechtloff Weising.
\end{frame}
\begin{frame}{Stable nonsymmetric Macdonald polynomials}
  Fix some \(r \in \Z_{>0}\). Add \(n\) empty ribbons after the first
  \(r\). Set variables \(x_{r+n} = x_{r+n+1} = \cdots = 0\).
\begin{equation*}
\begin{array}{c}
\begin{tikzpicture}[scale=.5]
...
\end{array}
\end{equation*}
On (signed) flagged ribbon LLTs, this causes bounds \(\geq r+n+1\) to
be no restriction and ribbons with bounds below ribbon \(\to 0\).
\end{frame}
\begin{frame}{The plethystic relationship}
  \begin{theorem}[Blasiak-Haiman-Morse-Pun-S., 2025+]
    \[
      \nsJ_{\eta|\lambda}(x;q,t) = t^{n((\eta;\lambda)_+)} \sum_R
      \big( \prod_{\ytableaushort{u,{}}} q^{a+1} t^l\big) \Gcal_{R,(1,2,\ldots,r)}(x;t^{-1})
    \]
  \end{theorem}\pause
  \begin{theorem}[Blasiak-Haiman-Morse-Pun-S., 2025+]
  \[\nsH_{\eta|\lambda}(x;q,t) = \Pi_r \nsJ_{\eta|\lambda}(x;q,t)\,.\]
\end{theorem}\pause
Follows from
\begin{itemize}
\item HHL style formula for both sides.
\item Stabilized ribbon LLT's satisfy \(\Gcal_{R,(1,2,\ldots,r)}(x;t^{-1}) = \Pi_r
  \Gcal^\pm_{R,(1,2,\ldots,r)}(x;t^{-1})\).
\end{itemize}
\end{frame}
...
\begin{frame}{Haglund-Haiman-Loehr combinatorics}
  \begin{itemize}
  \item For \(\beta \in \N^d\), the \emph{column diagram} of \(\beta\)
    consists of \(d\) bottom-justified columns of heights \(\beta_1,\ldots,\beta_d\).
  \item \(\arm(b) = \#\) of boxes above and in the same column as \(b\).
  \item \(\leg(b) = \)
  \end{itemize}
\end{frame}
\begin{frame}{Stabilization}
  \begin{itemize}
  \item On the flagged ribbon LLTs, the limit of this process is
    symmetric in \(x_{r+1},x_{r+2},\ldots\).
  \item Combinatorially, bound first \(r\) ribbons by
    \(1,\ldots,r\) and leave other ribbons unbounded.
  \item Consider \[
     \lim_{n \to \infty} \Ecal_{(\eta;0^n;\lambda)}(x_1,\ldots,x_n,0^k)
  \]
  \item Each LLT stabilizes as above, but some \(t\) coefficients get
    large!
  \item \(t\)-adically, those coefficients \(\to 0\).
  \item Result is symmetric in \(x_{r+1},x_{r+2},\ldots\)
  \end{itemize}
\end{frame}
\begin{frame}{Modern Macdonald polynomials}
  \begin{lemma}
    \(\Pi_r \Gcal^\pm_{\nubold,\bb}(x;t^{-1}) = \Gcal_{\nubold,\bb}\).
  \end{lemma}
  \begin{definition}
    The \emph{modern Macdonald polynomial} indexed by \(\eta \in
    \N^r\) and partition \(\lambda\) is \[
      \nsH_{\eta|\lambda}(x;q,t) = \Pi_r J_{\eta|\lambda}(x;q,t)
    \]
  \end{definition}
  \begin{theorem}
  \end{theorem}
