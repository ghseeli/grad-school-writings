...
\begin{document}
  \begin{frame}[t] % The whole poster is in one Beamer frame
      ...
      \begin{column}{.19\paperwidth}
        \begin{block}{Overview}
          ...
          \item Pieri rules determine the structure constants of these
            rings.
          \item Representatives are known for
            (co)homology of affine Grassmannian.
          \item Aim: develop similar picture for affine \(K\)-theory.
          \end{itemize}
        \end{block}
        ...
        \item Covers differ by boxes of same color.
          ...
          \\
          \[
            \emptyset \coveredby \ydiagram{1} \coveredby
            \ydiagram{2}*[*(red)]{1+1} \coveredby \ydiagram{1,3}*[*(red)]{0,1+1}*[*(blue)]{1,2+1}
            \coveredby \ydiagram{2,4}*[*(red)]{0,1+1}*[*(blue)]{1,2+1}
          \]
        \end{itemize}
        \textbf{Strong (Bruhat) Order}
        \begin{itemize}
        ...
        \item Covers differ by a ribbon \(+\) its copies.
          \ytableausetup{boxsize=0.45em}
              ...
              \]
            \item \textbf{Marked Cover:} Strong cover with selection of
              one ribbon \[
                \ydiagram{1,1,1,1,3,3,3,6,6} \coveredby
                \ydiagram{1,2,2,3,4,4,5,6,6}*[*(black)]{0,0,0,0,3+1,3+1,3+2}
              \]
         \end{itemize}
        \end{block}
      \end{column}
    ...
    with markings proceeding north to south.
        \begin{columns}[t]
          \begin{column}{.18\paperwidth}
        ...
        \begin{block}{Catalan Functions}
          ...
          \begin{block}{\(k\)-Schur Catalans}
            ...
            \textbf{Why use Catalan \(k\)-Schurs?}
              ...
              Dual Pieri rule \(\implies s^{(k)}_\lambda\) branching!
              \vspace{0.5cm}
              \[
                  s_\lambda^{(k)} \tikzmarknode{shift}{=} e_\ell^\perp
                  s_{\lambda+1^\ell}^{(k+1)} \tikzmarknode{pieri}{=}
                  \, \sum_{\mu} \, a_{\lambda \mu}^{(k)} s_{\mu}^{(k+1)}
              \]
              where \(a_{\lambda \mu}^{(k)}\)  counts strong vertical strips.
