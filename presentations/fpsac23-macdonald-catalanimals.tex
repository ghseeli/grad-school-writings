...
\title[Macdonald Catalanimals]{A Catalanimal formula for Macdonald polynomials} % The short title appears at the bottom of every slide, the full title is only on the title page
...
\medskip
FPSAC 2023\\ % Your institution for the title page
\medskip
Based on arXiv:2307.06517
}
\date{July 17, 2023} % Date, can be changed to a custom date
...
\begin{frame}{Symmetric functions and Schur functions}
  \begin{itemize}
  \item Let \(\Lambda(X)\) be the ring of symmetric functions in \(X =
    x_1,x_2,\ldots\) 
  ...
  \[
    s_\gamma = 
\begin{cases}
\sgn(\gamma+\rho) s_{\sort(\gamma+\rho) -\rho} & \text{if $\gamma + \rho$ has distinct parts,}\\
0                                                          & \text{otherwise,}
\end{cases}
  \]
\begin{itemize}
...
\end{itemize}
            ...
            % The \emph{symmetrization operator} \(\hbold_X \from
            % F[z_1^{\pm 1},\ldots,z_n^{\pm 1}] \to \Lambda(X)\) is given
            % by linearly extending
            % \vspace{-1mm}
            % \begin{equation*}
            %   \hbold_X(z_1^{\gamma_1} \cdots z_n^{\gamma_n}) =
            %   h_1^{\gamma_1}(X) \cdots h_n^{\gamma_n}(X) = h_\gamma(X) \,.
            % \end{equation*}
            % Set \(\zz = z_1,\ldots,z_n\) and \(\zz^\gamma =
            % z_1^{\gamma_1} \cdots z_n^{\gamma_n}\) for \(\gamma =
            % (\gamma_1,\ldots,\gamma_n) \in \Z^n\), so
            % \[
            %   \hbold_X(\zz^\gamma) = h_\gamma(X) \,.
            % \]
\end{frame}
...
\end{frame}
% \begin{frame}{Schur functions via Jacobi-Trudi}
%   For \(\gamma \in \Z^n\), define
%   \[
%     s_\gamma = \hbold_X\left(\zz^\gamma \prod_{\alpha_{ij} \in R_+}
%     (1-z_i/z_j)\right) \,.
%   \]
%   Then,
%   \[
...
% \item $\sgn(\beta) =$ sign of the shortest permutation taking $\beta$ to $\sort(\beta)$.
% \end{itemize}
% Define \(\sbold_X(f(\zz)) = \hbold_X\left( f(\zz) \prod_{\alpha \in
%     R_+} (1-\zz^\alpha) \right)\) so that \(\sbold_X(\zz^\gamma) = s_\gamma\).
% %   \vspace{-.8mm}
% % \begin{overlayarea}{\textwidth}{\textheight}
% % {\small
% % For $\lambda = (\lambda_1 \ge \lambda_2 \ge \cdots \ge \lambda_n)$, let  $\chi_\lambda(\zz) = \chi_\lambda(z_1,\dots,z_n)$
% % be the irreducible $GL_n$ character with highest weight \(\lambda\). 

% % \vspace{0mm}
% % %The Schur function $s_\lambda(z_1,z_2,\dots,z_n) = s_\lambda(\zz)$ is straightened as follows:
% % \begin{prop}[Schur function straightening]
% % For  $\gamma \in \Z^n$,
% % \vspace{-1.6mm}
% % \[\sigmabold (\zz^\gamma) =
% % \begin{cases}
% % \sgn(\gamma+\rho) \chi_{\sort(\gamma+\rho) -\rho}(\zz) & \text{if $\gamma + \rho$ has distinct parts,}\\
% % 0                                                          & \text{otherwise,}
% % \end{cases}
% % \]
% % \vspace{-4.5mm}
...
% % \end{itemize}
% % \end{prop}
% % When \(\lambda_n \geq 0\), the \emph{Schur function} is given by \(s_\lambda(\zz) = \chi_\lambda(\zz)\).

% % \only<1-2>{\onslide<2->{
% % $n = 4$, $\gamma = (3,1,2,5)$.
% % \vspace{-2mm}
% % \begin{align*}
% % &\gamma + \rho = (3,1,2,5) + (3,2,1,0) = (6,3,3,5) \text{ has a repeated part.}\\
% % &\text{Hence } \sigmabold(\zz^{3125}) = 0. \\
% % %s_{13}(\zz) &= -s_{22}(\zz)\\
% % \end{align*}}}
...
% % \vspace{-3mm}
% % \begin{align*}
% % &\gamma + \rho = (4,7,1,6) + (3,2,1,0) = (7,9,2,6)\\
% % &\sort(\gamma + \rho)  = (9,7,6,2)\\
% % &\sort(\gamma + \rho) - \rho  = (6,5,5,2)\\
% % &\text{Hence } \sigmabold(\zz^{4716}) = s_{6552}(\zz).
% % \end{align*}}
% % }
% % \end{overlayarea}
...
\begin{frame}{Modified Macdonald polynomials}
      \vspace{-1mm}
The \emph{modified Macdonald polynomials}  $\Htild_\mu = \Htild_\mu(X;q,t)$ are Schur positive
symmetric functions in \(X = x_1,x_2,\ldots\) over  $\Q(q,t)$.
\vspace{1.4mm}

They differ from the \emph{integral form Macdonald polynomials} by
$\Htild_\mu(X;q,t) = t^{\mathsf{n}(\mu )} J_{\mu}[\frac{X}{1-t^{-1}};q,t^{-1}]$.\pause

\vspace{2.5mm}

\vspace{.6mm}
$\Htild_{22} = s_4 + (q+t+qt)s_{31} + (q^2+t^2)s_{22} + (qt+ q^2t+qt^2)s_{211}+ q^2t^2 s_{1111}$
\[
\begin{tikzpicture}[scale=0.18]
\foreach \size in {7.1} {
\drawDg{1,1,1,1}{0}{0}
\drawDg{2,1,1}{1*\size}{0}
\drawDg{2,1,1}{0}{1*\size}
\drawDg{2,2}{0}{2*\size}
\drawDg{2,2}{2*\size}{0}
\drawDg{3,1}{2*\size}{1*\size}
\drawDg{3,1}{1*\size}{2*\size}
\drawDg{4}{2*\size}{2*\size}
\drawDg{3,1}{1.2*\size}{1.2*\size}
\drawDg{2,1,1}{.8*\size}{.8*\size}

\draw[thin] (-1.2,-1.2) -- (-1,2.5*\size);
\draw[thin] (-1.2,-1.2) -- (2.5*\size,-1);
\node at (-2.47,0.17*\size) {$t^0$};
\node at (-2.47,1.23*\size) {$t^1$};
\node at (-2.47,2.2*\size) {$t^2$};
\node at (0.2*\size,-2.47-.15) {$q^0$};
\node at (1.2*\size, -2.47-.15) {$q^1$};
\node at (2.16*\size, -2.47-.15) {$q^2$};
}
\end{tikzpicture}
\]
\end{frame}
\begin{frame}
  \frametitle{Modified Hall-Littlewood polynomials}

\vspace{-4mm}
When $q=0$, the modified Macdonald polynomials reduce to the
\emph{modified Hall-Littlewood polynomials}  $\Htild_\mu(X;0,t)$.
\vspace{4mm}

\(\Htild_{22}(X;0,t) = s_4 + t s_{31} + t^2 s_{22}\)

\[
\begin{tikzpicture}[scale=0.18]
\foreach \size in {8.1*16/20} {
\colorb{\drawDg{1,1,1,1}{0}{0}}
\colorgrgr{\drawDg{2,1,1}{1*\size}{0}}
\colorb{\drawDg{2,1,1}{0}{1*\size}}
\colorb{\drawDg{2,2}{0}{2*\size}}
\colorgrgr{\drawDg{2,2}{2*\size}{0}}
\colorgrgr{\drawDg{3,1}{2*\size}{1*\size}}
\colorgrgr{\drawDg{3,1}{1*\size}{2*\size}}
\colorgrgr{\drawDg{4}{2*\size}{2*\size}}
\colorgrgr{\drawDg{3,1}{1.2*\size}{1.2*\size}}
\colorgrgr{\drawDg{2,1,1}{.8*\size}{.8*\size}}

\draw[thin] (-1.2,-1.2) -- (-1,2.5*\size);
\draw[thin] (-1.2,-1.2) -- (2.5*\size,-1);
\node at (-2.47,0.17*\size) {\small $t^0$};
\node at (-2.47,1.23*\size) {\small $t^1$};
\node at (-2.47,2.2*\size) {\small $t^2$};
\node at (0.2*\size,-2.47-.12) {\small $q^0$};
\node at (1.2*\size, -2.47-.12) {\small $q^1$};
\node at (2.16*\size, -2.47-.12) {\small $q^2$};
}
\end{tikzpicture}
\]
\end{frame}
...
\vspace{-1mm}
{\small \begin{align*}
  \Htild^{(s)}_{\mu} := \omega \sigmabold  \Bigg( (z_1\cdots z_n)^s \,
\frac{
 \displaystyle\prod_{\alpha_{ij} \in R_\mu \setminus \widehat{R}_\mu }
 \raisebox{-1.4mm}{$\big(1- q^{\arm(b_i)+1} t^{-\leg(b_i)} z_i/z_j \big)$}
\displaystyle\prod_{\alpha \in \widehat{R}_{\mu}}
 \raisebox{-1.4mm}{$\big(1-q  t \zz^\alpha \big)$} } {\prod_{\alpha \in R_+} \big(1-q  \zz^\alpha\big)
\prod_{\alpha \in R_\mu} \big(1-t  \zz^\alpha\big)}\Bigg)
\end{align*}}


\vspace{-1.4mm}
\begin{conjecture}[Blasiak-Haiman-Morse-Pun-S.]
For any partition  $\mu$ and positive integer $\bbb$, the symmetric function
