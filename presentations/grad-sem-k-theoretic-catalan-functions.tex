...
\title[\(K\)-theory Catalans]{Schubert calculus and \(K\)-theoretic Catalan functions} % The short title appears at the bottom of every slide, the full title is only on the title page
...
\date{31 January 2020} % Date, can be changed to a custom date
...
\begin{frame}{Overview}
  \begin{enumerate}
  \item An overview of Schubert calculus
  \item Catalan functions: shedding new light on old problems
...
\begin{frame}{Next Step: Flag Variety}
  ...
  \item
   ...
   \item Decomposes into Schubert varieties indexed by \(w \in S_n\).\pause
    ...
    \(\S_w \in \Z[x_1, \ldots, x_n]\). \pause

    \item Structure constants \(\S_w \S_u = c_{w u}^v \S_v \) are
...
\begin{frame}{Peterson Isomorphism}
  ...
  \end{itemize}
  \begin{block}{Upshot}
    Computations for Schubert polynomials can be moved into symmetric functions.
  \end{block}
\end{frame}
\begin{frame}{\(k\)-Schur functions}
  \begin{itemize}
  \item \(s_\lambda^{(k)}\) for \(\lambda_1 \leq k\) a basis for
    \(\Z[s_1, s_2, \ldots, s_k]\). \pause
  \item Schubert representatives for homology of affine Grassmannian,
    \(\Gr_{SL_{k+1}}\). \pause
  \item Has a tableaux formulation from which some properties were
    proven. \pause
  \item Branching
      \[s_{\ydiagram{2,2}}^{(2)} =
      ...
      \item Has geometric meaning for embedding of affine
        Grassmannians.\pause
...
\begin{frame}{Root Ideals}
  ...
  by Dyck path above the diagonal.
          ...
          \begin{block}{Catalan Function~\cite{CH,pany,catalans}}
            ...
            $$
            H(\Psi;\gamma)(x) = \prod_{(i,j)\in\Delta^+_\ell
...
\begin{frame}{Catalan functions}
    Key ingredient of branching proof:\pause
...
\]
\pause Branching is a special case of Pieri:
\[
...
\begin{frame}{Dual Grothendieck polynomials}
  ...
  \item Satisfies Pieri rule on ``set-valued strips''
    \ytableausetup{aligntableaux=bottom} \pause
    \begin{align*}
      g_{1^2} g_{3,2} = 
      & g_{43}
      + g_{421}
      ...
      -2 g_{321}
      + g_{31}\\
      & \ydiagram{2,3}*[*(red)]{2+1,3+1},
      \ydiagram{0,2,3}*[*(red)]{1,0,3+1},
      \ydiagram{0,2,3}*[*(red)]{1,2+1},
      \ydiagram{2,3}*[*(red)]{0,3+1}*[*(lightgray)]{1+1},
      \ydiagram{2,3}*[*(red)]{2+1}*[*(lightgray)]{0,2+1},
      \ydiagram{0,2,3}*[*(red)]{1}*[*(lightgray)]{0,1+1},
      \ydiagram{0,2,3}*[*(red)]{1}*[*(lightgray)]{0,0,2+1},
    ...
    \item Dual to Grothendieck polynomials: Schubert representatives
      for \(K^*(Gr(m,n))\)
  \end{itemize}
\end{frame}
\begin{frame}{\(K\)-\(k\)-Schur functions}
  ...
  \item Satisfies Pieri rule on ``affine set-valued strips''\pause
    ...
    \begin{align*}
      g_1 g_{\ydiagram{1,1,3}}^{(2)} & = g_{\ydiagram{1,1,2,4}}^{(2)} -
      2g_{\ydiagram{1,1,3}}^{(2)} \\
      \begin{ytableau}
      ...
      \end{ytableau}
    \end{align*}\pause
  \item Conjecture: \(g_\lambda^{(k)}\) have branching into
    \(g_\mu^{(k+1)}\).\pause
    \begin{block}{Problem}
...
\begin{frame}{\(K\)-theoretic Peterson isomorphism}
  ...
  \begin{theorem}[Blasiak-Morse-S., 2020]
    ...
    then \(g_\lambda^{(k)} = g_\lambda \implies\) conjecture is true
    for \(w\) a Grassmannian permutation.
  \end{theorem}\pause
...
\begin{frame}{Future Directions}
    ...
    \item Combinatorially describe \(g_\lambda^{(k)} = \sum_\mu ??
      s_\mu^{(k)}\).\pause
    \item Describe the image of \(\G_w^Q\) under Peterson isomorphism
      for all \(w \in S_{k+1}\).
    \end{enumerate}
\end{frame}
