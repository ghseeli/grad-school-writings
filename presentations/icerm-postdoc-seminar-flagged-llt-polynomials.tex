...
\begin{frame}{Flagged Tableaux}
  ...
  \end{center}
\end{frame}
\begin{frame}{Flagged Schur functions}
  \begin{itemize}
  ...
  \item E.g., \(s_{21,13}(x_1,x_2,x_3) = x_1^2x_2 + x_1^2 x_3 = h_2(x_1)h_1(x_1,x_2,x_3)-h_3(x_1)\)\pause
...
\begin{frame}{Recovering Symmetric Functions}
  ...
  \item For \emph{Weyl symmetrization operator} \(\pi_{w_0}\), we have \[
        ...
        \pi_{w_0}(\Dcal_\alpha) = \Dcal_{\sort(\alpha)},
      \]
      \[
        \pi_{w_0}(\Acal_\alpha) =
        \begin{cases}
          \Acal_\alpha & \alpha \text{ a partition},\\
          0 & \text{else.}
        \end{cases}
    \]
  \end{itemize}
...
%
        \draw[thick] (4,3) grid (6,5); \only<1-2>{ \node at (0.5, 1.5)
...
\begin{frame}{Applications}
  ...
  \item Tewari-Wilson-Zhang define chromatic nonsymmetric polynomials
    associated to \(d \times d\) Dyck paths starting with \(r\) north steps, \(\chi_{\bb,\pi}\). We show \[
      \chi_{(1,\ldots,r),\pi} = (1-t)^{r-d} \Gcal^\pm_{\nu(\pi),(1,\ldots,r)}(x;t)\,.
    \]
  \end{enumerate}
\end{frame}
\end{document}
