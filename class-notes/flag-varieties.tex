...
\begin{proof}
  ...
  \begin{center}
    \begin{tabular}{c|c}
      ...
      x \in \P(V), v_1 \in \ell_x, v_2 \in \R^3 / \ell_x\) \\
...
\begin{lem}
  Since \[
    H^*(\P(E);\R) = H^*(M;\R) \otimes H^*(\CP^{n-1};\R)
  \]
  we have that \[
...
\begin{proof}
  ...
  \]
  and so the result follows immediately from dimension counting of the
  tensor product.
\end{proof}
...
\end{rmk}
\begin{defn}
  Given a complex vector space of dimension \(n\), we define the
...
\begin{proof}
  ...
  computations of the flag manfold Poincar\'{e} polynomials \ref{poincare-of-flag-manifold}, \[
    P_t(F(V)) = P_t(F(f^*Q)) = P_t(G_k(V)) \frac{(1-t^2) \cdots (1-t^{2(n-k)})(1-t^2)
