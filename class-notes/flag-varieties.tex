...
\begin{prop}[Properties of Gysin maps]\label{props-of-gysin-maps}
  ...
  \item Closed embedding: Given a \(G\)-invariant closed embedding
    ...
    \item (self-intersection), for \(\alpha \in H_G^*(Y)\) we have
      ...
      of the normal bundle. \todo{normal or conormal?}
    \end{enumerate}
  ...
  \begin{minipage}{1.0\linewidth}
    \begin{multicols}{2}
      \begin{tikzpicture}[scale=2,cap=round,>=latex]
        ...
        % Modified by George Seelinger
    
        % draw the coordinates
        % \draw[->] (-1.5cm,0cm) -- (1.5cm,0cm) node[right,fill=white]
        % {};
        % \draw[->] (0cm,-1.5cm) -- (0cm,1.75cm)
        % node[above,fill=white]
        % {};
        ...
        % draw the points
        \filldraw[black] (0:1cm) circle(0.4pt); \filldraw[black]
        (90:1cm) circle(0.4pt);
    
        % label the points
        \draw (0cm,1.25cm) node[fill=white] {$\left[ 1,0 \right]$};
        \draw (1.25cm,0cm) node[above=1pt] {$\left[ 0,1 \right]$};

        % draw the tangent line
        \draw (1cm,-1.5cm) -- (1cm,1.75cm) node[above,fill=white]
        {$T_{p_1}\P^1$};
      \end{tikzpicture}
...
\(p \in X^T\).
\begin{thm}[First Locatlization Theorem]\label{first-localization-theorem}
...
\begin{defn}
  we say that \(X\) is \de{equivariantly formal} if \(H_T^*(X)\) is a
...
\end{rmk}
\begin{thm}[Atiyah-Bott-Berline-Vergne, ABBV Integration Formula]\label{integration-formula}
...
\subsection{The Grassmannian}
\begin{defn}
  Let \(V = \C^n\). Then, we will define \de{the Grassmannian of
    \(V\)} to be \(\Gr_d(V) := \{E \subset V
  \st \dim W = d\}\). 
\end{defn}
\begin{prop}
  There is an embedding of the Grassmannian, called the
  \de{Pl\"{u}cker embedding} given by
  \begin{align*}
...
\begin{defn}
  ...
  \begin{enumerate}
  \item the \de{Schubert cells} in \(\Gr_d(V)\) are \(C_I := BE_I\)
  \item the \de{Schubert varieties} in \(\Gr_d(V)\) are \(X_I :=
    \ov{C_I}\). 
  \end{enumerate}
\end{defn}
\begin{rmk}
...
\begin{defn}
  ...
  \item We define the flag
    ...
    \]
  \item We define \de{Schubert cells} in \(\Fl(V)\) by \(C_w := BF_w =
    BwB/B\).
  \item We define \de{Schuber varieties} in \(\Fl(V)\) by \(X_w =
    \ov{C_w}\).
  \end{enumerate}
\end{defn}
\begin{rmk}
...
\begin{proof}
  ...
  \todo{Show at least the other direction.}
\end{proof}
\begin{defn}
  For \(w \in \Sym_n\),
  \begin{enumerate}
  \item we define the \de{opposite Schubert cell}
  ...
  \item we define the \de{opposite Schubert variety} \(X^w := \ov{C^w}
...
\begin{defn}
  ...
  manifold. Then, the \de{fundamental class of \(M\)} (in homology) is
...
\begin{defn}
  ...
  \item Given Schubert variety \(X_w \subset \Fl(V)\), we define its
    \de{Schubert class} \([X_w] \in H^{m-2\ell(w)}\) to be the
  ...
  \item Given an opposite Schubert variety \(X^w\), we define its
    \de{Schubert class} \([X^w] \in H^{2 \ell(w)}\) to be its
    representative in cohomology following the same procedure as
    above. 
  \end{enumerate}
\end{defn}
\begin{prop}
 Both \(\{[X_w]\}\) and \(\{[X^w]\}\) form \(\Z\)-bases of
 \(H^*(\Fl(V))\). 
\end{prop}
\begin{proof}
  Since \(\Fl(V)\) is a disjoint union of its Schubert classes and the
real dimensions of all its cells are concentrated in even degree, its
integral homology groups will not exhibit any torsion. Thus, the
Schubert classes form a basis of the integral cohomology which is a
free \(Z\)-module with rank \(n!\).  
\end{proof}
\begin{bibdiv}
  \begin{biblist}
    \bib{bott-tu}{book}{
    ...
    \bib{brion}{article}{
      ...
      journal={Topics in Cohomological Studies of Algebraic Varieties}
      pages={33--85}
      year={2005}
    }
    \bib{fulton}{article}{
      ...
      year={2007}
      note={Notes by Dave Anderson. Available at
