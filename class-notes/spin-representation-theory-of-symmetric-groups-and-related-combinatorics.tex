...
\end{example}
\section{A Morita Superequivalence}
...
\otimes \Cl_n\) and \(\H_n\).
\section{A Double Cover \(\tilde{B}_n\)}
...
\begin{cor}
  ...
  \]
\end{cor}
Now, the conjugacy class \(C_{\rho^+,\rho^-}\) is even if \(k\) is
even for \(\underbrace{b_{i_1} b_{i_2} \ldots b_{i_k}}_{\in \Z_2^n} \sigma \in
C_{\rho^+,\rho^-}\).
\begin{thm}[Read]
  \cite{cheng-wang}*{Theorem 3.31}
  \begin{enumerate}
  \item Even \(C_{\rho^+,\rho^-}\) splits if and only if \(\rho^+ \in
    \OP_n\) and \(\rho^- = \emptyset\)
  \item Odd \(C_{\rho^+,\rho^-}\) splits if and only if \(\rho^+ =
