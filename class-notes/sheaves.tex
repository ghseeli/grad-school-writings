...
\begin{example}
  ...
  \item Given \(Y\) as another topological space, recall that, given a
    ...
    define the \de{presheaf of sections} (with respect to \(\pi\)) by defining, for
    \(U \subset X\), \[
...
\begin{thm}
  ...
  functor sending \(\F\) to \(\F^+\) is called \de{sheafification}.
\end{thm}
\begin{proof}
  Postponed for now.
\end{proof}
...
% \begin{lem}
%   Given a presheaf \(\F\), we have \(\F_x \isom \F^+_x\).
% \end{lem}
% \begin{proof}
%   By definition of \(\F^+\), 
% \end{proof}
...
\begin{proof}
    ...
    this is a presheaf.
    % As in intermeidate step, we will show that
    % \(C^+ = 0\) in both directions.
    ...
    %   \F_x \to \G_x \to C_x \to 0
    % \]
    % which gives us that \(C_x = G_x/\phi_x(F_x)\).
    % % Furthermore,
    % % sheafification does not change the stalks by \ref{}, so \(C_x^+ =
    % % C_x = G_x/\phi_x(F_x)\).
    % Thus, \(\phi_x \from \F_x \to \G_x\) be surjective for all
    % \(x\) if and only if \(C_x = 0\) for all \(x\).
...
\begin{defn}
  ...
  \de{local homeomorphism} if every \(e \in E\) has a neighborhood
